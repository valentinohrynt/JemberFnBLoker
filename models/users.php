            ...
            if ( $result[ 'status' ] === 'active' ) {
                ...
                if ( $verify ) {
                    ...
                    if ( $existingTokenQuery->num_rows > 0 ) {
                        $conn->query( "UPDATE user_tokens SET token = '$token', expires_at = '$token_expires_at' WHERE user_id = '$user_id'" );
                    } else {
                        $conn->query( "INSERT INTO user_tokens (user_id, token, expires_at, created_at) VALUES ('$user_id', '$token', '$token_expires_at', '$current_time')" );
                    }
